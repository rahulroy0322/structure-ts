...
import {
  getRequestBody,
  getTransformedBody,
} from '../../../src/stratus-ts/question/body/main'
...
describe('# getTransformedBody', () => {
  describe('# basic body', () => {
    ...
    it('# should return empty body/files for unknown content-type', async () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith(`invalid content type "text/plain"`)

      errorSpy.mockRestore()
    })
  })

  describe('# maptipart/form-data', () => {
    ...
    it('# should throw on multipart/form-data with non-POST method', async () => {
      ...
      await expect(() => getTransformedBody(req)).rejects.toThrow(
        'only "post" methode can handel maptipart data!'
      )
    })
  })
})
