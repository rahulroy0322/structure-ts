...
import { getMaltiPartBody } from '../../../src/stratus-ts/question/body/maltipart'
...
describe('# getMaltiPartBody', () => {
  it('# should parses simple multipart form with text fields', async () => {
  ...
  it('# should parses multipart form with a file upload', async () => {
  ...
  it('# should parses multipart with both field and file', async () => {
    const body = buildMultipartBody([
    ...
    assert.strictEqual(result.files.resume.data.toString(), 'This is my resume')
  })

  describe('# invalid body', () => {
    it('# throws on missing boundary', async () => {
      const req = createMockRequest({}, Buffer.from(''))
      await expect(() => getMaltiPartBody(req)).rejects.toThrow(
        /No boundary found/
      )
    })
  })
