...
description: Complete guide to configuring your Structure-Ts application
---

{/* # Settings Configuration */}

The `src/settings.ts` file is the central configuration hub for your Structure-Ts application. This guide explains every configuration option in simple, easy-to-understand language.
...
- Port 2000: Structure-Ts default
...
1. Structure-Ts reads this array
...
3. Restart your server—routes are now active!
...
const DATABASE: DatabaseType = {
  ...
  database: process.env.DB_NAME || 'structure_app',
};
```
...
```typescript
import { render } from 'structure-ts';

const homeController: ControllerType = (request, reply) => {
  render(reply, 'pages/home', {
    title: 'Home Page',
    user: { name: 'Alice' },
  });
};
...
- **[Routing](/guides/routing/)** - Define URL patterns
- **[Controllers](/guides/controllers/)** - Handle requests
- **[Database Setup](/guides/database-setup/)** - Detailed database guide
- **[Models](/guides/models/)** - Create database models
