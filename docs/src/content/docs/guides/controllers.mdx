...
```typescript
// Route: route.get('/:id', getUser)

const getUser: ControllerType = (question, reply) => {
  const { id } = question.params;  // Get :id from URL
...
const protectedController: ControllerType = (question, reply) => {
  const authHeader = question.headers['authorization'];
  const contentType = question.headers['content-type'];
...
Use any HTTP status code:

```typescript
reply.status(418).json({
  success: false,
  error: "I'm a teapot",  // Yes, this is a real status code!
});
...
- **[Models](/guides/models/)** - Connect controllers to databases
- **[Rendering](/guides/rendering/)** - Render HTML templates from controllers
- **[Database Setup](/guides/database-setup/)** - Configure database connections
